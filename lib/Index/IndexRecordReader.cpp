...
struct IndexRecordReader::Implementation {
  ...
  void readDecl(unsigned Index, IndexRecordDecl &RecD) {
    ...
    RecD.DeclID = Index + 1;
    RecD.SymInfo.Kind =
        getSymbolKind((indexstore_symbol_kind_t)read(Record, I));
    RecD.SymInfo.SubKind =
        getSymbolSubKind((indexstore_symbol_subkind_t)read(Record, I));
    RecD.SymInfo.Lang =
  ...
  bool foreachOccurrence(
    ...
    public:
      OccurBitVisitor(
          ...
          ArrayRef<const IndexRecordDecl *> RelatedDeclsFilter,
          function_ref<bool(const IndexRecordOccurrence &)> Receiver)
          : BitstreamVisitor(Stream), Reader(Reader), DeclsFilter(DeclsFilter),
  ...
  bool foreachOccurrenceInLineRange(
    ...
    return foreachOccurrence(None, None,
...
IndexRecordReader::IndexRecordReader() : Impl(*new Implementation()) {}

IndexRecordReader::~IndexRecordReader() { delete &Impl; }
