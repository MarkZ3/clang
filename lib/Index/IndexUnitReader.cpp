...
typedef function_ref<bool(const IndexUnitReader::DependencyInfo &)>
    DependencyReceiver;
typedef function_ref<bool(const IndexUnitReader::IncludeInfo &)>
    IncludeReceiver;
  ...
  StreamVisit visitRecord(unsigned BlockID, unsigned RecID,
    ...
    case UNIT_PATHS_BLOCK_ID:
      ...
      case UNIT_PATH: {
        ...
        Reader.Paths.emplace_back(
            Kind, BitPathComponent(DirOffset, DirSize),
            BitPathComponent(FilenameOffset, FilenameSize));
      } break;
      case UNIT_PATH_BUFFER:
        Reader.PathsBuffer = Blob;
        Reader.WorkingDir =
            Reader.getPathFromBuffer(WorkDirOffset, WorkDirSize);
        Reader.OutputFile =
            Reader.getPathFromBuffer(OutputFileOffset, OutputFileSize);
        Reader.SysrootPath =
            Reader.getPathFromBuffer(SysrootOffset, SysrootSize);
...
bool IndexUnitReaderImpl::init(std::unique_ptr<MemoryBuffer> Buf,
  ...
  if (Stream.Read(8) != 'I' || Stream.Read(8) != 'D' || Stream.Read(8) != 'X' ||
      Stream.Read(8) != 'U') {
  ...
  IndexUnitBlockBitstreamVisitor Visitor(
      ...
      [&](RecordDataImpl &Record, StringRef Blob) {
        ...
        return Receiver(IndexUnitReader::DependencyInfo{
            DepKind, IsSystem, Name, PathBuf.str(), ModuleName, FileSize,
            ModTime});
      });
...
bool IndexUnitReaderImpl::foreachInclude(IncludeReceiver Receiver) {
  ...
  IndexUnitBlockBitstreamVisitor Visitor(
      UNIT_INCLUDE, IncludeCursor, [&](RecordDataImpl &Record, StringRef Blob) {
        unsigned I = 0;
...
void IndexUnitReaderImpl::constructFilePath(SmallVectorImpl<char> &PathBuf,
  ...
  sys::path::append(
      PathBuf, getPathFromBuffer(Path.Dir.Offset, Path.Dir.Size),
...
StringRef IndexUnitReaderImpl::getModuleName(int ModuleIndex) {
  ...
  return StringRef(ModuleNamesBuffer.data() + ModInfo.NameOffset,
                   ModInfo.NameSize);
}
...
IndexUnitReader::createWithFilePath(StringRef FilePath, std::string &Error) {
  ...
  bool Err = Impl->init(std::move(*ErrOrBuf),
...
IndexUnitReader::~IndexUnitReader() { delete IMPL; }
...
StringRef IndexUnitReader::getTarget() const { return IMPL->getTarget(); }

bool IndexUnitReader::hasMainFile() const { return IMPL->hasMainFile(); }

bool IndexUnitReader::isSystemUnit() const { return IMPL->isSystemUnit(); }

bool IndexUnitReader::isModuleUnit() const { return IMPL->isModuleUnit(); }
