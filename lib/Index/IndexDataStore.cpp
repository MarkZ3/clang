...
bool IndexDataStoreImpl::startEventListening(
    llvm::function_ref<AbstractDirectoryWatcher::CreateFnTy> createFn,
    bool waitInitialSync, std::string &Error) {
  if (DirWatcher) {
  ...
  auto OnUnitsChange = [localUnitEventHandlerData](
                           ArrayRef<AbstractDirectoryWatcher::Event> Events,
                           bool isInitial) {
    ...
    for (const AbstractDirectoryWatcher::Event &evt : Events) {
      ...
      case AbstractDirectoryWatcher::EventKind::Added:
        K = IndexDataStore::UnitEventKind::Added;
        break;
