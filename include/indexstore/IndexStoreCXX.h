...
public:
  ...
  public:
    ...
    Kind getKind() const {
      ...
      Kind K;
      switch (c_k) {
      case INDEXSTORE_UNIT_EVENT_ADDED:
        K = Kind::Added;
        break;
    ...
    timespec getModificationTime() const {
      return indexstore_unit_event_get_modification_time(obj);
    }
  };
  ...
  void getUnitNameFromOutputPath(StringRef outputPath,
    ...
    size_t nameLen = indexstore_store_get_unit_name_from_output_path(
        obj, buf.c_str(), nameBuf.data(), nameBuf.size());
  ...
  llvm::Optional<timespec> getUnitModificationTime(StringRef unitName,
                                                   std::string &error) {
    llvm::SmallString<64> buf = unitName;
    int64_t seconds, nanoseconds;
    indexstore_error_t c_err = nullptr;
    bool err = indexstore_store_get_unit_modification_time(
        obj, buf.c_str(), &seconds, &nanoseconds, &c_err);
    if (err && c_err) {
      error = indexstore_error_get_description(c_err);
      indexstore_error_dispose(c_err);
      return llvm::None;
...
public:
  ...
  time_t getModificationTime() {
    return indexstore_unit_dependency_get_modification_time(obj);
  }
